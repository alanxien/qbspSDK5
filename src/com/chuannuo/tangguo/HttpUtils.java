package com.chuannuo.tangguo;

import com.chuannuo.tangguo.net.BinaryHttpResponseHandler;
import com.chuannuo.tangguo.net.TGHttpClient;
import com.chuannuo.tangguo.net.TGHttpResponseHandler;
import com.chuannuo.tangguo.net.JsonHttpResponseHandler;
import com.chuannuo.tangguo.net.RequestParams;

public class HttpUtils {
	private static TGHttpClient client = new TGHttpClient(); // 实例化对象
	static {
		client.setTimeout(11000); // 设置链接超时，如果不设置，默认为10s
	}

	public static void get(String urlString, TGHttpResponseHandler res) // 用一个完整url获取一个string对象
	{
		client.get(urlString, res);
	}

	public static void get(String urlString, RequestParams params,
			TGHttpResponseHandler res) // url里面带参数
	{
		client.get(urlString, params, res);
	}

	public static void get(String urlString, JsonHttpResponseHandler res) // 不带参数，获取json对象或者数组
	{
		client.get(urlString, res);
	}

	public static void get(String urlString, RequestParams params,
			JsonHttpResponseHandler res) // 带参数，获取json对象或者数组
	{
		client.get(urlString, params, res);
	}

	public static void get(String uString, BinaryHttpResponseHandler bHandler) // 下载数据使用，会返回byte数据
	{
		client.get(uString, bHandler);
	}

	public static TGHttpClient getClient() // 获取HCKHttpClient对象
	{
		return client;
	}
	
	public static void post(String urlString, RequestParams params, TGHttpResponseHandler res){
		client.post(urlString, params, res);
	}
}
